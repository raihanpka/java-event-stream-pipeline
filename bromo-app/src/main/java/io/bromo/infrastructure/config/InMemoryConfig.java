...
/**
 * In-memory adapters for the dev profile. Lets the app boot without Cassandra,
 * PostgreSQL, or Redis. Real adapters (Cassandra, Postgres, Redis) are used
 * in the prod profile and supersede these by Spring's primary bean resolution.
 *
 * <p>{@code @Profile("dev")} keeps the entire config out of production classpath
 * scanning. Do not add {@code @ConditionalOnMissingBean} here - per Spring docs,
 * that condition is only reliable on auto-configuration classes.
 */
@Configuration
@Profile("dev")
public class InMemoryConfig {
    ...
    @Bean
    public QueryService queryService(EventRepository eventRepository, MetricRepository metricRepository) {
    ...
    static final class InMemoryMetricRepository implements MetricRepository {
        ...
        @Override
        public List<Metric> findByTypeAndMetric(String type, String metricName, Instant since, Instant until) {
            return store.stream()
